...
1. `npx @shahboura/harness gate --id <gate> --present --run <run>`
...
4. `npx @shahboura/harness gate --id <gate> --decide --run <run>`
...
5. Outcomes:
   - decision recorded → `npx @shahboura/harness cursor --to <next>` (forward or the
...
6. Security gate only: a `defer` decision → the decide result carries a
   ...
   `npx @shahboura/harness provider --op work_item.create --title
   ...
   `npx @shahboura/harness log-event --json
...
7. Publish the mirror after the crossing — **once per preflighted repo**
   ...
   workspace: `npx @shahboura/harness publish-mirror --repo <preflighted-repo-path> --run <run>`.
...
8. ⟨approve-impl⟩ only, BEFORE presenting: `npx @shahboura/harness
